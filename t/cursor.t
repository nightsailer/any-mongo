...
# use AnyMongo;
# 
# my $conn = AnyMongo->new_connection(host => 'mongodb://127.0.0.1');
# 
# my $ns = 'test.anymongo';
# my $q = {};
# my ($limit,$skip) = (2,0);
...
# );
# 
# my $result=$cursor->count;
# 
# ok($result >0,'count');
# 
# diag('$cursor->next:'.Dumper($result));
...
my $conn;
eval {
    my $host = "mongodb://127.0.0.1";
    if (exists $ENV{MONGOD}) {
        $host = $ENV{MONGOD};
    }
    $conn = AnyMongo->new_connection(host => $host);
};
...
my $_tmp = $coll->query->next;
# warn Dumper($_tmp);
is($_tmp, undef, 'test undef');
